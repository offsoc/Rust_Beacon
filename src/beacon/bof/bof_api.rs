...
pub const RDATA_SECTION_RELOC: u16 = 1024;
pub const DATA_SECTION_RELOC: u16 = 1025;
pub const EXE_SECTION_RELOC: u16 = 1026;
pub const DYNAMIC_FUNC_RELOC: u16 = 1027;

pub const MULTI_RELOC: u16 = 1028;
pub const RELOC_REL32: u16 = 20;
pub const RELOC_ADDR32: u16 = 6;
pub const RELOC64_REL32: u16 = 4;
pub const RELOC_UNK_10: u16 = 10;

#[repr(C)]
pub struct RELOCATION {
    pub r_type: u16,              // relocation type - r_type
    pub sof: SOF,                 // section or function
    pub r_offset: u32,            // virtual address of the item to be relocated - r_offset
    pub e_offset_in_section: u32, // value of the symbol - e_offset_in_section
}
...
pub fn get_beacon_function_ptr(name: &str) -> Result<usize, Box<dyn std::error::Error>> {
    match name {
        // Data
...
impl Carrier {
    ...
    pub unsafe fn append_char_array(&mut self, s: *mut c_char, len: c_int) {
        ...
        for i in 0..len {
            let value = ptr::read_unaligned(s.add(i as usize) as *const c_char);
            buffer.push(value); // Store directly as `c_char`
        }

        self.output.extend_from_slice(&buffer);
        self.offset = self.output.len() - buffer.len();
    }
...
pub static mut OUTPUT: Carrier = Carrier::new();
